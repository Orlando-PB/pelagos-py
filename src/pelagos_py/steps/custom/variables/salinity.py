...
class AdjustSalinity(BaseStep, QCHandlingMixin):
    ...
    parameter_schema = {
        "filter_window_size": {
            "type": int,
            "default": 21,
            "description": "Window length over which the running average filter is applied."
        },
        "qc_handling_settings": {
            ...
            "description": "Rules for filtering bad data before adjustment."
        }
    }
    ...
    def correct_ct_lag(self):
        ...
        processed_count = 0
        max_profiles = 100
        filter_size = self.parameters.get("filter_window_size", 21)
            ...
            if len(valid_times) > 0:
                ...
                if duration >= np.timedelta64(1, 'h') and len(valid_times) > 3 * filter_size:
                    if getattr(self, 'diagnostics', False) and self._ct_cost_data is None:
                        optimal_lag, cost_data = compute_optimal_lag(profile, filter_size, self.time_col, return_cost_data=True)
                        self._ct_cost_data = cost_data
                    else:
                        optimal_lag = compute_optimal_lag(profile, filter_size, self.time_col)
                    
                    self.per_profile_optimal_lag[i, :] = [profile_number, optimal_lag]
                    processed_count += 1

        # Apply shifts
        valid_data_mask = self.data["CNDC"].notnull() & self.data[self.time_col].notnull()
        if not np.any(valid_data_mask):
            self.log("No valid CNDC data found. Skipping CT lag correction.")
            return

        lags = self.per_profile_optimal_lag[~np.isnan(self.per_profile_optimal_lag[:, 1]), 1]
    ...
    def generate_diagnostics(self):
        ...
        if np.any(valid_indices):
            ...
            ax_lag.scatter(profs_plot, lags_plot, c=COLOUR_COMBINED, s=12, alpha=0.6, label=label_text, zorder=2)
            ax_lag.axhline(self.ct_lag_median, color=COLOUR_COMBINED, linestyle='--', lw=1.5, zorder=3)
        ...
        if len(uncorr["DEPTH"].dropna(dim="N_MEASUREMENTS")) > 0:
            ...
            ax_sal.plot(p_new, uncorr["DEPTH"].values, c=COLOUR_COMBINED, ls="", marker=".", ms=1.5, alpha=0.7)
            sal_legend.append(mlines.Line2D([], [], color=COLOUR_COMBINED, marker=".", ls="", markersize=4, label="Corr Combined"))
        ...
        if len(t_valid) > 0:
            ...
            ax_diff.plot(elapsed_days, temp_diff, color=COLOUR_CORR_T, marker=".", ls="", ms=1, alpha=0.4, label="Temp Diff")
            ax_diff_c.plot(elapsed_days, cndc_diff, color=COLOUR_CORR_C, marker=".", ls="", ms=1, alpha=0.4, label="CNDC Diff")
